...
def validate_and_merge_json(response_jsons: tuple):
    """Merge multiple JSON responses into a single dictionary."""
    output_json = {}
    for response_json in response_jsons:
        try:
            # If already a dict, just merge
            if isinstance(response_json, dict):
                output_json.update(response_json)
                continue
            # Otherwise sanitize and parse
            if isinstance(response_json, str):
                sanitized = response_json.replace("None", "null").replace(
                    "'", '"'
                )
                parsed_obj = json.loads(sanitized)
                output_json.update(parsed_obj)
            else:
                # For unsupported types, log and skip
                logger.warning(
                    f"Unsupported response_json type: {type(response_json)}"
                )
        except json.JSONDecodeError as e:
            logger.error(f'Error: "{response_json}" is not valid JSON: {e}')
...
def validate_category(response, target_id):
    """Validate that a category ID exists and is a child category."""
    if target_id is None:
        return None

    target_item = next(
        (item for item in response if item["id"] == target_id), None
    )
...
def validate_account(response, target_id):
    """Validate that an account ID exists in the response."""
    if target_id is None:
        return None

    return (
        target_id
        if any(item["id"] == target_id for item in response)
        else None
    )


def validate_labels(response, target):
    """Validate that label IDs exist in the response."""
...
def validate_response(response_json, api_categories, api_labels, api_accounts):
    """Validate and normalize the complete response JSON."""
    validated = {}

    # accountId
    account_id = response_json.get("accountId")
    validated["accountId"] = (
        account_id if isinstance(account_id, int) and account_id >= 0 else None
    )
    validated["accountId"] = validate_account(
        api_accounts, validated["accountId"]
    )

    # amount
    amount = response_json.get("amount")
    validated["amount"] = (
        amount if isinstance(amount, (int, float)) and amount > 0 else 0.0
    )
    ...
    validated["categoryId"] = validate_category(
        api_categories, validated["categoryId"]
    )
    ...
    validated["businesses"] = (
        businesses if isinstance(businesses, list) else []
    )
